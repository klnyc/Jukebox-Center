...
export const getAlbums = (genre) => {
  return async (dispatch) => {
    try {
      const { data } = genre
        ? await Axios.get(`/api/albums/${genre}`)
...
export const authenticate = (state, history) => {
  ...
  return async (dispatch) => {
    try {
      const { data } = await Axios.post(`/api/user/${method}`, {
        email,
        password,
      });
...
export const addToCart = (albumId, quantity) => {
  return async (dispatch) => {
    try {
      ...
      exist
        ? dispatch(setError("Already added to cart"))
...
export const getGuestCart = () => {
  window.localStorage.cart =
    window.localStorage.cart ||
    JSON.stringify({ id: true, address: "", albums: [] });
